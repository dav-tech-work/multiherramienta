window.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const response = await fetch('/dns', {
                ...
                body: JSON.stringify({ dominio, tipo })
            });

            if (!response.ok) {
                const data = await response.json();
                throw new Error(data.error || 'Error en la consulta DNS');
            }

            const data = await response.json();
            mostrarResultado(data.resultado);
        } catch (error) {
            mostrarError(error.message || 'Error al realizar la consulta DNS');
    ...
    function mostrarCargando() {
        ...
        resultadoElement.innerHTML = 'Consultando...';
    }
